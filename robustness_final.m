...
%% Dynamics with changed mass
Change_perc = -0.994;
m = 4.78*(1+Change_perc);
lr = 0.18;
lf = 0.18;
...
CombVal_rob = matlabFunction(Comb,'vars',{qq,uu},"File","CombVal_rob");
...
for ii=1:steps-100
    ...
    % linearization and discretization from original model
    ...
    % non linear dynamics to update state using ode45 with new mass
    [~,qqall] = ode45(@(t,q) CombVal_rob(qqval,Input),[timesteps(ii) timesteps(ii+1)], qqval);
